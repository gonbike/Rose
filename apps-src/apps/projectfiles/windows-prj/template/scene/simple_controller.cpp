...
/*
 * How to use...
 * simple_controller controller(game.core_cfg(), game.video());
...
simple_controller::simple_controller(const config& top_config, CVideo& video)
...
void simple_controller::app_create_display()
{
	gui_ = new simple_display(*this, units_, video_, map_);
}
...
void simple_controller::app_execute_command(int command, const std::string& sparam)
{
	...
	switch (command) {
		...
		case HOTKEY_ZOOM_IN:
			gui_->set_zoom(gui_->hex_size());
			break;
		case HOTKEY_ZOOM_OUT:
			gui_->set_zoom(- gui_->hex_size() / 2);
			break;
