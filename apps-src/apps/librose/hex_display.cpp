...
hex_display::hex_display(base_controller* controller, CVideo& video, const tmap* map, size_t num_reports)
	: display(game_config::tile_hex, controller, video, map, num_reports)
{
}
...
const map_location hex_display::map_2_loc(int x, int y) const
{
	...
	if(nearest_hex != NULL) {
		...
		const int centerx = (loc_2_screen_x(res) - map_area().x + xpos_) + hex_size()/2 - hex_width();
		const int centery = (loc_2_screen_y(res) - map_area().y + ypos_) + hex_size()/2 - hex_size();
...
void hex_display::draw_border(const map_location& loc, const int xpos, const int ypos)
{
	...
	if(loc.x == -1 && loc.y == -1) { // top left corner
		...
		if(loc.x%2 == 0) {
			drawing_buffer_add(LAYER_BORDER, loc, xpos, ypos + zoom_/2,
				border_.corner_image_top_right_odd, image::SCALED_TO_ZOOM);
		} else {
		...
		if(loc.x%2 == 1) {
			...
			drawing_buffer_add(LAYER_BORDER, loc, xpos, ypos,
				border_.corner_image_bottom_right_odd, image::SCALED_TO_ZOOM);
		}
	...
	// Now handle the sides:
		...
		if(loc.x%2 == 1) {
			drawing_buffer_add(LAYER_BORDER, loc, xpos, ypos,
				border_.border_image_top_even, image::SCALED_TO_ZOOM);
		} else {
			drawing_buffer_add(LAYER_BORDER, loc, xpos, ypos + zoom_/2,
		...
		if(loc.x%2 == 1) {
			drawing_buffer_add(LAYER_BORDER, loc, xpos, ypos,
				border_.border_image_bottom_even, image::SCALED_TO_ZOOM);
		} else {
			drawing_buffer_add(LAYER_BORDER, loc, xpos, ypos + zoom_/2,
