...
bool effect::render()
{
	...
	if (loc_.x != -1 && loc_.y != -1) {
		if (disp->shrouded(loc_)) {
			...
			set_location(
				disp->loc_2_screen_x(loc_) + disp->hex_size() / 2,
				disp->loc_2_screen_y(loc_) + disp->hex_size() / 2,
				true);
		}
	}
...
int add(int x, int y, bool screen, const std::string& image, const map_location& loc, ORIENTATION orientation, bool infinite)
{
	...
	for (; itor != items.end(); ++itor) {
		...
		image::tblit blit(str, image::SCALED_TO_ZOOM);
		blit.width = image::calculate_scaled_to_zoom(surf->w);
		blit.height = image::calculate_scaled_to_zoom(surf->h);
