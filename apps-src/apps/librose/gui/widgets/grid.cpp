...
tpoint tgrid::fill_placeable_width(const int width)
{
	...
	for (unsigned row = 0; row < rows_; ++row) {
		...
		if (cols_ == 1) {
			...
			ret = cell.widget_->fill_placeable_width(width - cell_border_space(cell).x);
			if (ret.x != twidget::npos) {
...
*/
			...
			for (unsigned col = 0; col < cols_; ++col) {
				...
				ret = cell.widget_->fill_placeable_width(placeable_width - cell_border_space(cell).x);
...
void tgrid::place(const tpoint& origin, const tpoint& size)
{
	...
	if (best_size.x <= size.x && best_size.y <= size.y) {
		...
		if (size.x > best_size.x) {
			std::pair<int, int> restrict_width_factor = std::make_pair(twidget::npos, twidget::npos);
			if (cols_ > 1) {
				for (int at = 0; at < children_vsize_; at ++) {
					if (children_[at].widget_->restrict_width() && col_width_[at % cols_]) {
						...
						VALIDATE(restrict_width_factor.first == twidget::npos, null_str);
			...
			if (restrict_width_factor.first != twidget::npos) {
...
void tgrid::popup_new_window()
{
	for (int n = 0; n < children_vsize_; n ++) {
		tchild& child = children_[n];

		child.widget_->popup_new_window();
	}
}
