		...
		, const unsigned indention_step_size) const
{
	...
	if (parent_node_) {
		if (panel_->gc_height_ == twidget::npos) {
			best_size = panel_->get_best_size();
			panel_->gc_width_ = best_size.x;
			panel_->gc_height_ = best_size.y;
		} else {
			best_size.x = panel_->gc_width_;
			best_size.y = panel_->gc_height_;
		}
	}
...
void ttree_view_node::set_origin(const tpoint& origin)
{
	...
	VALIDATE(origin == tree_view_.content_grid()->get_origin(), null_str);

	// Using layout_children seems to fail.
	set_origin(tree_view().indention_step_size_, origin);
}

unsigned ttree_view_node::set_origin(const unsigned indention_step_size, tpoint origin)
{
	if (tree_view().left_align_) {
	...
	if (panel_) {
		panel_->set_origin(tpoint(tree_view_.content_grid()->get_x(), origin.y));
	}

	if (!is_root_node()) {
		origin.x += indention_step_size;
		origin.y += panel_->get_height();
	}
	...
	for (std::vector<ttree_view_node*>::iterator it = children_.begin(); it != children_.end(); ++ it) {
		ttree_view_node& node = **it;
		origin.y += node.set_origin(indention_step_size, origin);
	}
...
unsigned ttree_view_node::place(const unsigned indention_step_size, tpoint origin, unsigned width)
{
	...
	// best_size.x = width;
	best_size.x = tree_view_.content_grid()->get_width();
	if (!is_root_node()) {
		panel_->grid().widget(0, 0)->set_layout_size(tpoint(best_size.x - width, best_size.y));
		panel_->place(tpoint(tree_view_.get_x(), origin.y), best_size);
		origin.x += indention_step_size;
	...
	twidget::set_size(tpoint(best_size.x, origin.y - offset));
...
void ttree_view_node::init_grid(tgrid* grid, const std::map<std::string, std::string>& data)
{
	assert(grid);
