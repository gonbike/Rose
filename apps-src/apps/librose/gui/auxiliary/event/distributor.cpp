...
void tdistributor::signal_handler_sdl_mouse_motion(const tpoint& coordinate, const tpoint& coordinate2)
{
	...
	// posix_print("signal_handler_sdl_mouse_motion------coordinate(%i, %i), mouse_focus_: %p\n", coordinate.x, coordinate.y, mouse_focus_);
...
{
	VALIDATE(hover_timer_ != INVALID_TIMER_ID, null_str);
	remove_timer(hover_timer_);
...
/*
	posix_print("[#%i]signal_handler_sdl_button_down------(%i, %i), mouse_focus_: %p, mouse_click_: %p, window_id(%s)\n",
		...
		, const tpoint& coordinate)
{
	...
	posix_print("------[#%i]signal_handler_sdl_button_up[#%i], mouse_click_: %p, window_id(%s)\n", times, button_, distributor_.mouse_click_,
		distributor_.owner_.id().empty()? "nill": distributor_.owner_.id().c_str());
}
