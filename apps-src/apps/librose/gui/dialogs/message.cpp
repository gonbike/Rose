...
void tportrait_message::pre_show(CVideo& video, gui2::twindow& window)
{
	...
	tfloat_widget* item = window.find_float_widget("float_portrait");
	item->set_visible(true);
	item->widget->set_label(portrait_);

	item = window.find_float_widget("float_incident");
	item->set_visible(true);
...
{
	show_message(video, _("Error"), message, tmessage::ok_button, "hero-256/0.png", "");
}
