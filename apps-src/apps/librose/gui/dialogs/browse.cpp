...
void tbrowse::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	if (game_config::mobile) {
		...
		list.set_did_row_double_click(boost::bind(&tbrowse::item_double_click, this, boost::ref(window), _1, _2));
	}
	list.set_did_row_changed(boost::bind(&tbrowse::item_selected, this, boost::ref(window), _1, _2));
...
void tbrowse::add_row(twindow& window, tlistbox& list, const std::string& name, bool dir)
{
	...
	list_item_item.insert(std::make_pair("type", get_browse_icon(dir)));
