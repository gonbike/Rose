...
enum {BLITM_NONE, BLITM_LOC, BLITM_RECT, BLITM_LINE, BLITM_SURFACE}; // type of blit material
struct tblit
{
	tblit()
		: type(BLITM_NONE)
	...
	explicit tblit(const surface& surf, int _width, int _height, const SDL_Rect& _clip = SDL_Rect(), int _x = 0, int _y = 0)
		: type(BLITM_SURFACE)
		, x(_x)
		, y(_y)
		, clip(_clip)
	...
	explicit tblit(const locator& loc, const TYPE loc_type, int _width = 0, int _height = 0, const SDL_Rect& _clip = SDL_Rect(), int _x = 0, int _y = 0)
		: type(BLITM_LOC)
		, x(_x)
		, y(_y)
		, clip(_clip)
	...
	explicit tblit(uint32_t color, int width, int height, int x = 0, int y = 0)
		: type(BLITM_RECT)
		, x(x)
		, y(y)
		, clip(create_rect(0, 0, 0, 0))
	...
	explicit tblit(uint32_t color, const SDL_Point& p1, const SDL_Point& p2)
		: type(BLITM_LINE)
		, x(p1.x)
		, y(p1.y)
		, clip(create_rect(0, 0, 0, 0))
...
class tblits: private boost::noncopyable
{
public:
	tblits()
		: buf(NULL)
	...
	void merge_with(tblits& that);
	void push_back(const tblit& blit);
	tblit& back() { return *(buf[vsize - 1]); }
	const tblit& at(int _at) const { return *(buf[_at]); }

	void clear(bool free_buf);
...
void blit_integer_blits(std::vector<image::tblit>& blits, const int canvas_width, const int canvas_height, const int x, const int y, int integer);
void generate_pip_blits(std::vector<image::tblit>& blits, int width, int height, const std::string& bg, const std::string& fg);
void generate_integer2_blits(std::vector<image::tblit>& blits, int width, int height, const std::string& img, int integer, bool greyscale);
