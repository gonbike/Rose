...
void render_blit(SDL_Renderer* renderer, const image::tblit& blit, const int xpos, const int ypos)
{
	...
	SDL_Rect *srcrectArg = (srcrect.x | srcrect.y | srcrect.w | srcrect.h)? &srcrect : NULL;
	if (blit.type == image::BLITM_LOC) {
		image::render_locator_texture(renderer, blit, dstrect.x, dstrect.y, srcrectArg);

	} else if (blit.type == image::BLITM_SURFACE) {
		dstrect.w = blit.width;
		dstrect.h = blit.height;
		render_surface(renderer, blit.surf, srcrectArg, &dstrect);
...
void blit_integer_blits(std::vector<image::tblit>& blits, const int canvas_width, const int canvas_height, const int x, const int y, int integer)
{
	...
	do {
		...
		blits.push_back(image::tblit(ss.str(), image::UNSCALED));
		blit = &blits.back();
		blit->x = dst_clip.x;
		blit->y = dst_clip.y;
...
void generate_pip_blits(std::vector<image::tblit>& blits, int width, int height, const std::string& bg, const std::string& fg)
{
	...
	blits.push_back(image::tblit(result, width, height));
}
...
void generate_integer2_blits(std::vector<image::tblit>& blits, int width, int height, const std::string& img, int integer, bool greyscale)
{
	...
	if (use_surface) {
		...
		blits.push_back(image::tblit(surf, width, height));
	} else {
		blits.push_back(image::tblit(img, image::UNSCALED, width, height));
	}
