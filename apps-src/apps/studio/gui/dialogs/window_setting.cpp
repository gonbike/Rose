...
void twindow_setting::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	for (std::vector<std::string>::const_iterator it = labels.begin(); it != labels.end(); ++ it) {
		tcontrol& widget = bar_->create_child(null_str, null_str, index ++);
		widget.set_label(*it);
...
bool twindow_setting::save_float_widget(twindow& window, const int at)
{
	...
	if (!verify_formula(disp_, "width's fomula", item.x[0].first)) {
		return false;
	}
	...
	if (!utils::isinteger(text_box->label().c_str())) {
		show_id_error(disp_, "width's offset", utils::errstr);
		return false;
	}
	...
	if (!verify_formula(disp_, "height's fomula", item.x[0].first)) {
		return false;
	}
	...
	if (!utils::isinteger(text_box->label().c_str())) {
		show_id_error(disp_, "height's offset", utils::errstr);
		return false;
	}
...
void twindow_setting::switch_float_widget(twindow& window)
{
	...
	text_box->set_value(str_cast(item.w.second));
	...
	text_box = find_widget<ttext_box>(float_widget_page, "float_widget_height_offset", false, true);
	text_box->set_maximum_chars(5);
	text_box->set_value(str_cast(item.h.second));
...
{
	float_widget_at_ = (int)widget->cookie();
